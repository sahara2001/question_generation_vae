...
#    hs = train_params.get('hs') or allowed_params.get('hs')
    ...
    model_name = 'word2vec_datagrand_' + str(dimension) + 'dim'
...
def preprocess(tokenizer_dir, raw_corpus_dir, corpus_dir):
    logger = logging.getLogger(u'处理语料')
    logger.info(u'加载分词器中...')
    tokenizer = Tokenizer(tokenizer_dir)
    logger.info(u'读取语料中...')
    with open(raw_corpus_dir,'r',encoding='utf-8') as f:
        texts = f.readlines()
    logger.info(u'分词中...')
    corpus = [' '.join(tokenizer.tokenize(item.replace('_','|').replace('\n','') + '|')) + '\n' for item in texts]
    logger.info(u'保存分词结果中...')
    with open(corpus_dir,'w',encoding='utf-8') as f:
        f.writelines(corpus)
    
if __name__ == '__main__':
    preprocess_, train_ = [False, True]
    if preprocess_:
        tokenizer_dir = root_dir+'/data/gen_data/dic.txt'
        raw_corpus_dir = root_dir + '/data/raw_data/corpus.txt'
        corpus_dir = root_dir + '/data/gen_data/corpus_cut.txt'
        preprocess(tokenizer_dir, raw_corpus_dir, corpus_dir)
    if train_:
        corpus_dir = root_dir + '/data/gen_data/corpus_cut.txt'
        model_dir = root_dir + '/word2vec'
        train_params = {
            'dimension': 128,
            'replace': True,
            'window': 5,
